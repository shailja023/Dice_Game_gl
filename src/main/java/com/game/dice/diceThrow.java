package main.java.com.game.dice;

import main.java.com.game.utilities.SortByPoints;
import main.java.com.game.utilities.SortByWonPositions;
...
public class diceThrow {
	...
	public static ArrayList<ArrayList<Integer>> composeGame() {
		...
		for (int i = 0; i < N; i++) {
			...
			// Flag info -> 0: Blocked(2 consecutive 1s) | 1: Can play clean | 2: Got 1 in
			// last chance | >2: Player has won.
		...
		while (playersRemaining > 1) {
			for (int i = 0; i < N; i++) {
				...
				if (list.get(i).get(1) == 0) {
					System.out.println("Player-" + list.get(i).get(0)
							+ ", you are blocked for this turn as you rolled 2 consecutive 1s");
					list.get(i).set(1, 1);
					continue;
				...
				if (list.get(i).get(2) >= M) {
					...
					list.get(i).set(1, 1);
					System.out.println(
							"Please roll the dice again, Player-" + list.get(i).get(0) + " as you have got 6.");
					d = diceRoll(i, list);
					if (list.get(i).get(2) >= M) {
						playersRemaining = win(list, i, playersRemaining);
						continue;
	...
	private static int win(ArrayList<ArrayList<Integer>> list, int i, int playersRemaining) {
		System.out.println(
				"You have won Player-" + list.get(i).get(0) + " and your rank is " + (N - playersRemaining + 1));
		playersRemaining--;
	...
	private static void printRanks(ArrayList<ArrayList<Integer>> list) {
		...
		for (int i = 0; i < listRemainingPlayers.size(); i++) {
			System.out.println("Rank:" + (i + pos + 1) + " is Player-" + listRemainingPlayers.get(i).get(0)
					+ " with points: " + listRemainingPlayers.get(i).get(2));
		}
	}
}